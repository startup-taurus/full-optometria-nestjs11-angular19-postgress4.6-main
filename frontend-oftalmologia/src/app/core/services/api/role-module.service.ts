...
export class RoleModuleService {
  private API_URL = `${environment.apiBaseUrl}/role-modules`
  ...
  public assignModuleToRole(assignData: {
    roleId: string
    moduleId: string
    isEnabled?: boolean
  ...
  public removeModuleFromRole(
    roleId: string,
    moduleId: string
  ): Observable<ApiResponse<any>> {
    const endpoint = `${this.API_URL}/remove/${roleId}/${moduleId}`
  ...
  public getRoleModules(roleId: string): Observable<ApiResponse<any>> {
    const endpoint = `${this.API_URL}/role/${roleId}`
    return this._httpClient.get<ApiResponse<any>>(endpoint)
  }

  public getAllRoleModules(): Observable<ApiResponse<any>> {
